...
4b. Take @iteles's total pay, go to `transferwise` and set up a payment to her account (already saved as a contact), mark it as `"DWYL SALARY" + MONTH`, and note down the amount (it will include a transfer fee)
...
5a. Add Ines's `transferwise` payment to the list, but make sure that you use tranferwise's standard reference
